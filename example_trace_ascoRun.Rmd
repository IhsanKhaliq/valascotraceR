...
billa <- vector(mode = "list", length = 20)
time_run <- vector(mode = "numeric", length = length(billa))
...
cat("\nAverage model runtime =", mean(time_run))
```

Inspect the severity of the paddock sub-quadrats.

```{r heatmap}
billa <- rbindlist(lapply(billa, tidy_trace), idcol = TRUE)
...
# cross section plot at row 10 (x == 10) on day 145

ggplot(billa_plot, aes(x = y, y = infectious_gp, colour = Run)) +
  geom_hex() +
  scale_fill_viridis_c(option = "E") +
  theme_classic()
```
...
plot(
  obs_billa[location == "Billa Billa", mean(V1), by = assessment_date],
  pch = 16,
  #     col = "blue",
  lwd = 2,
  ylab = "Proportion of infected quadrats",
  xlab = "Disease progress over time"
)
